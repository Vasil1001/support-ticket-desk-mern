...
export default function Ticket() {
  ...
  return (
    <div>
      ...
      <div className="text-xl">Product: {ticket.product}</div>
      <div className="flex flex-col p-3 border border-gray-400 bg-gray-200">
        <h3 className="font-bold">Description of Issue</h3>
        <p>{ticket.description}</p>
        <p className="mt-3 text-sm text-gray-500">Ticket ID: {ticket._id}</p>
      </div>
      <div>
        <div className="flex justify-between">
          <h1 className="flex ml-1 mb-1 items-end font-bold text-3xl">Notes</h1>
          {ticket.status !== "closed" && (
            <button
              className="flex p-2 px-4 mb-3 items-center bg-black text-white rounded-md gap-2"
              onClick={openModal}
            >
              <FaPlus /> Add Note
            </button>
